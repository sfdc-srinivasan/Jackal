package com.hellblazer.jackal.multiprocess.controller.gui;
...
import org.smartfrog.services.anubis.partition.test.controller.gui.GraphicControllerConfig;
...
@Import({GraphicControllerConfig.class, GossipSnoopConfig.class,
        TestCfg.class, GossipHeartbeatAndDiscoveryConfig.class,
        GossipTestCfg.class})
public class TestControllerConfig {
    private final static Logger log = LoggerFactory
            .getLogger(TestControllerConfig.class);
    ...
    public ExecutorService gossipDispatchers(Identity partitionIdentity) {
        ...
        return Executors.newCachedThreadPool(new ThreadFactory() {
            ...
            public Thread newThread(Runnable target) {
                Thread t = new Thread(target, String.format(
                        "Gossip Dispatcher[%s] for node[%s]", count++, id));
                t.setDaemon(true);
